 ...
 */
#include "soundsensor-server-stub.h"
 ...
 */
static void soundsensor_object_init(SoundSensorObject* obj) {
	printf("Called");
	g_assert(obj != NULL);
...
G_DEFINE_TYPE(SoundSensorObject, soundsensor_object, G_TYPE_OBJECT)
/**
...
void soundsensor_object_emitSignal(SoundSensorObject* obj,
	...
	SoundSensorObjectClass* klass = SOUNDSENSOR_OBJECT_GET_CLASS(obj);
	...
	/* Check that the given num is valid (abort if not).
	   ...
	   development work). */
	g_assert((num < E_SOUNDSENSOR_OBJECT_SIGNAL_COUNT) && (num >= 0));
...
{
	printf("soundsensor1: get_status called.\n");
	g_assert(obj != NULL);

		/* Copy the current first soundsensor to caller specified memory. */
	//if (obj->m_cur_status == DEVICE_STATUS_ERROR)
	//{
	//	soundsensor_object_emitSignal(obj, E_SOUNDSENSOR_OBJECT_SIGNAL_ERROR, "error");
	//}
