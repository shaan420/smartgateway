 ...
 */
#include "lighting-server-stub.h"
 ...
 */
static void lighting_object_init(LightingObject* obj) {
	printf("Called");
	g_assert(obj != NULL);
 ...
 */
static void lighting_object_class_init(LightingObjectClass* klass) {
	...
	/* Since all signals have the same prototype (each will get one
	   ...
	   Since the index goes from 0 to E_LIGHTING_OBJECT_SIGNAL_COUNT-1, we just specify
	   the signal names into an array and iterate over it.

	   Note that the order here must correspond to the order of the
	   enumerations before. */
	const gchar* signalNames[E_LIGHTING_OBJECT_SIGNAL_COUNT] = {
		SIGNAL_CHANGED_STATUS,
		SIGNAL_ERROR };
	/* Loop variable */
	int i;

	printf("lighting_object_class_init Called\n");
	g_assert(klass != NULL);
	...
	for (i = 0; i < E_LIGHTING_OBJECT_SIGNAL_COUNT; i++) {
		guint signalId;
	 ...
	 * the D-Bus. */
	dbus_g_object_type_install_info(LIGHTING_TYPE_OBJECT,
			&dbus_glib_lighting_object_object_info);
...
G_DEFINE_TYPE(LightingObject, lighting_object, G_TYPE_OBJECT)
/**
...
void lighting_object_emitSignal(LightingObject* obj,
	...
	LightingObjectClass* klass = LIGHTING_OBJECT_GET_CLASS(obj);
	...
	/* Check that the given num is valid (abort if not).
	   ...
	   development work). */
	g_assert((num < E_LIGHTING_OBJECT_SIGNAL_COUNT) && (num >= 0));

	printf("Emitting signal id %d, with message '%s'\n", num, message);
...
gboolean lighting_object_get_status(LightingObject* obj, gchar * *cur_status, GError** error) 
{
	...
	/* Copy the current first lighting to caller specified memory. */
	/*
...
{
	printf("lighting1: set_status called.\n");
	g_assert(obj != NULL);

//	if (atoi(obj->m_cur_status[0]) != new_status) 
//	{
		/* Change the lighting. */
		// No need to change this here as it will be updated from the reader thread path
	//	obj->m_cur_status = new_status;
