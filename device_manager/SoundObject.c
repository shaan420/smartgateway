 ...
 */
static void sound_object_init(SoundObject* obj) {
	printf("Called");
	g_assert(obj != NULL);
	obj->m_cur_status = DEVICE_STATUS_OFF;
}
 ...
 */
static void sound_object_class_init(SoundObjectClass* klass) {
	...
	/* Since all signals have the same prototype (each will get one
	   ...
	   Since the index goes from 0 to E_SOUND_OBJECT_SIGNAL_COUNT-1, we just specify
	   the signal names into an array and iterate over it.

	   Note that the order here must correspond to the order of the
	   enumerations before. */
	const gchar* signalNames[E_SOUND_OBJECT_SIGNAL_COUNT] = {
	...
	for (i = 0; i < E_SOUND_OBJECT_SIGNAL_COUNT; i++) {
		guint signalId;
	 ...
	 * the D-Bus. */
	dbus_g_object_type_install_info(SOUND_TYPE_OBJECT,
			&dbus_glib_sound_object_object_info);
...
G_DEFINE_TYPE(SoundObject, sound_object, G_TYPE_OBJECT)
/**
...
void sound_object_emitSignal(SoundObject* obj,
	...
	SoundObjectClass* klass = SOUND_OBJECT_GET_CLASS(obj);
	...
	/* Check that the given num is valid (abort if not).
	   ...
	   development work). */
	g_assert((num < E_SOUND_OBJECT_SIGNAL_COUNT) && (num >= 0));
...
{
	printf("sound1: get_status called.\n");
	g_assert(obj != NULL);

		/* Copy the current first sound to caller specified memory. */
	//if (obj->m_cur_status == DEVICE_STATUS_ERROR)
	//{
	//	sound_object_emitSignal(obj, E_SOUND_OBJECT_SIGNAL_ERROR, "error");
	//}
...
{
	printf("sound: set_status called. new_status=%d\n", new_status);
	g_assert(obj != NULL);


	SoundDevice *dev = static_cast<SoundDevice *>(obj->m_dev_ptr);
	if (NULL != dev)
	{
		printf("Writing to physical device\n");
		dev->Write(&new_status, sizeof(new_status));

		/* Emit the "changed_sound1" signal. */
		//printf("Publishing new new_status\n");
		//sound_object_emitSignal(obj, E_SOUND_OBJECT_SIGNAL_CHANGED_STATUS, "new_status");
	}
	else
	{
