 ...
 */
#include "temperaturesensor-server-stub.h"
 ...
 */
static void temperaturesensor_object_init(TemperatureSensorObject* obj) {
	printf("Called");
	g_assert(obj != NULL);
 ...
 */
static void temperaturesensor_object_class_init(TemperatureSensorObjectClass* klass) {
	...
	/* Since all signals have the same prototype (each will get one
	   ...
	   Since the index goes from 0 to E_TEMPERATURESENSOR_OBJECT_SIGNAL_COUNT-1, we just specify
	   the signal names into an array and iterate over it.

	   Note that the order here must correspond to the order of the
	   enumerations before. */
	const gchar* signalNames[E_TEMPERATURESENSOR_OBJECT_SIGNAL_COUNT] = {
	...
	for (i = 0; i < E_TEMPERATURESENSOR_OBJECT_SIGNAL_COUNT; i++) {
		guint signalId;
	 ...
	 * the D-Bus. */
	dbus_g_object_type_install_info(TEMPERATURESENSOR_TYPE_OBJECT,
			&dbus_glib_temperaturesensor_object_object_info);
...
G_DEFINE_TYPE(TemperatureSensorObject, temperaturesensor_object, G_TYPE_OBJECT)
/**
...
void temperaturesensor_object_emitSignal(TemperatureSensorObject* obj,
	...
	TemperatureSensorObjectClass* klass = TEMPERATURESENSOR_OBJECT_GET_CLASS(obj);
	...
	/* Check that the given num is valid (abort if not).
	   ...
	   development work). */
	g_assert((num < E_TEMPERATURESENSOR_OBJECT_SIGNAL_COUNT) && (num >= 0));
...
{
	printf("temperaturesensor1: get_status called.\n");
	g_assert(obj != NULL);

		/* Copy the current first temperaturesensor to caller specified memory. */
	//if (obj->m_cur_status == DEVICE_STATUS_ERROR)
	//{
	//	temperaturesensor_object_emitSignal(obj, E_TEMPERATURESENSOR_OBJECT_SIGNAL_ERROR, "error");
	//}
