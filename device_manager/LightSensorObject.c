 ...
 */
#include "lightsensor-server-stub.h"
 ...
 */
static void lightsensor_object_init(LightSensorObject* obj) {
	printf("Called");
	g_assert(obj != NULL);
 ...
 */
static void lightsensor_object_class_init(LightSensorObjectClass* klass) {
	...
	/* Since all signals have the same prototype (each will get one
	   ...
	   Since the index goes from 0 to E_LIGHTSENSOR_OBJECT_SIGNAL_COUNT-1, we just specify
	   the signal names into an array and iterate over it.

	   Note that the order here must correspond to the order of the
	   enumerations before. */
	const gchar* signalNames[E_LIGHTSENSOR_OBJECT_SIGNAL_COUNT] = {
	...
	for (i = 0; i < E_LIGHTSENSOR_OBJECT_SIGNAL_COUNT; i++) {
		guint signalId;
	 ...
	 * the D-Bus. */
	dbus_g_object_type_install_info(LIGHTSENSOR_TYPE_OBJECT,
			&dbus_glib_lightsensor_object_object_info);
...
G_DEFINE_TYPE(LightSensorObject, lightsensor_object, G_TYPE_OBJECT)
/**
...
void lightsensor_object_emitSignal(LightSensorObject* obj,
	...
	LightSensorObjectClass* klass = LIGHTSENSOR_OBJECT_GET_CLASS(obj);
	...
	/* Check that the given num is valid (abort if not).
	   ...
	   development work). */
	g_assert((num < E_LIGHTSENSOR_OBJECT_SIGNAL_COUNT) && (num >= 0));
...
{
	printf("lightsensor1: get_status called.\n");
	g_assert(obj != NULL);

		/* Copy the current first lightsensor to caller specified memory. */
	//if (obj->m_cur_status == DEVICE_STATUS_ERROR)
	//{
	//	lightsensor_object_emitSignal(obj, E_LIGHTSENSOR_OBJECT_SIGNAL_ERROR, "error");
	//}
