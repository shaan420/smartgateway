#ifndef __TEST_LIGHTING_DEVICE_HPP__
#define __TEST_LIGHTING_DEVICE_HPP__

#include "LightingDevice.hpp"

class testLightingDevice : public LightingDevice
{
	private:
		int m_cur_value;
		int m_prev_value;

	protected:
		int RecvFromDevice();
		int SendToDevice();

		testLightingDevice() {}

	public:
		testLightingDevice(const string name, const string desc, DeviceConf_t conf)
			: LightingDevice(name, desc, conf)
		{
		}

		~testLightingDevice() {}

		int OnInit();
		void OnError();
		void OnChange();
		void OnSendDone();
		bool HasChanged();
		void SetDeviceParams();
		void GetDeviceParams();
		int ReadShared(void **mem);
		int Write(void *mem, int len);
		int Read(void *mem, int len);
		int UpdateDataManager();
		int PrepareOutputData(string& value);
};

#endif
