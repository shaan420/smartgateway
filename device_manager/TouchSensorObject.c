 ...
 */
#include "touchsensor-server-stub.h"
 ...
 */
static void touchsensor_object_init(TouchSensorObject* obj) {
	printf("Called");
	g_assert(obj != NULL);
 ...
 */
static void touchsensor_object_class_init(TouchSensorObjectClass* klass) {
	...
	/* Since all signals have the same prototype (each will get one
	   ...
	   Since the index goes from 0 to E_TOUCHSENSOR_OBJECT_SIGNAL_COUNT-1, we just specify
	   the signal names into an array and iterate over it.

	   Note that the order here must correspond to the order of the
	   enumerations before. */
	const gchar* signalNames[E_TOUCHSENSOR_OBJECT_SIGNAL_COUNT] = {
	...
	for (i = 0; i < E_TOUCHSENSOR_OBJECT_SIGNAL_COUNT; i++) {
		guint signalId;
	 ...
	 * the D-Bus. */
	dbus_g_object_type_install_info(TOUCHSENSOR_TYPE_OBJECT,
			&dbus_glib_touchsensor_object_object_info);
...
G_DEFINE_TYPE(TouchSensorObject, touchsensor_object, G_TYPE_OBJECT)
/**
...
void touchsensor_object_emitSignal(TouchSensorObject* obj,
	...
	TouchSensorObjectClass* klass = TOUCHSENSOR_OBJECT_GET_CLASS(obj);
	...
	/* Check that the given num is valid (abort if not).
	   ...
	   development work). */
	g_assert((num < E_TOUCHSENSOR_OBJECT_SIGNAL_COUNT) && (num >= 0));
...
{
	printf("touchsensor1: get_status called.\n");
	g_assert(obj != NULL);

		/* Copy the current first touchsensor to caller specified memory. */
	//if (obj->m_cur_status == DEVICE_STATUS_ERROR)
	//{
	//	touchsensor_object_emitSignal(obj, E_TOUCHSENSOR_OBJECT_SIGNAL_ERROR, "error");
	//}
