 ...
 */
#include "notificationagent-server-stub.h"
...
gboolean notificationagent_object_new_event(NotificationAgentObject* obj, gchar * new_event, GError** error) 
{
	...
	g_assert(obj != NULL);
		...
		if (obj->m_new_event != new_event) {
		/* Change the notificationagent. */
		obj->m_new_event = new_event;

		NotificationAgentDevice *dev = static_cast<NotificationAgentDevice *>(obj->m_dev_ptr);
		if (NULL != dev)
		{
			printf("Writing to physical device\n");
			dev->Write(&new_event, sizeof(new_event));
...
gboolean notificationagent_object_new_subscriber(NotificationAgentObject* obj, gchar * event,gchar * new_sub, GError** error) 
{
	...
	g_assert(obj != NULL);
		...
		{
			printf("Writing to physical device\n");
			dev->Write(&event, sizeof(event));

			/* Emit the "changed_notificationagent1" signal. */
			printf("Publishing new event\n");
			notificationagent_object_emitSignal(obj, E_NOTIFICATIONAGENT_OBJECT_SIGNAL_CHANGED_STATUS, "event");
		}
		else
		{
			printf("Error contacting physical device\n");
	...
	if (obj->m_new_sub != new_sub) {
		/* Change the notificationagent. */
		obj->m_new_sub = new_sub;

		NotificationAgentDevice *dev = static_cast<NotificationAgentDevice *>(obj->m_dev_ptr);
		if (NULL != dev)
		{
			printf("Writing to physical device\n");
			dev->Write(&new_sub, sizeof(new_sub));
...
gboolean notificationagent_object_publish_event(NotificationAgentObject* obj, gchar * event_params, GError** error) 
{
	...
	g_assert(obj != NULL);
		...
		if (obj->m_event_params != event_params) {
		/* Change the notificationagent. */
		obj->m_event_params = event_params;

		NotificationAgentDevice *dev = static_cast<NotificationAgentDevice *>(obj->m_dev_ptr);
		if (NULL != dev)
		{
			printf("Writing to physical device\n");
			dev->Write(&event_params, sizeof(event_params));
