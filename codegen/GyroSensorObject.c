 ...
 */
#include "gyrosensor-server-stub.h"
 ...
 */
static void gyrosensor_object_init(GyroSensorObject* obj) {
	printf("Called");
	g_assert(obj != NULL);
 ...
 */
static void gyrosensor_object_class_init(GyroSensorObjectClass* klass) {
	...
	/* Since all signals have the same prototype (each will get one
	   ...
	   Since the index goes from 0 to E_GYROSENSOR_OBJECT_SIGNAL_COUNT-1, we just specify
	   the signal names into an array and iterate over it.

	   Note that the order here must correspond to the order of the
	   enumerations before. */
	const gchar* signalNames[E_GYROSENSOR_OBJECT_SIGNAL_COUNT] = {
	...
	for (i = 0; i < E_GYROSENSOR_OBJECT_SIGNAL_COUNT; i++) {
		guint signalId;
	 ...
	 * the D-Bus. */
	dbus_g_object_type_install_info(GYROSENSOR_TYPE_OBJECT,
			&dbus_glib_gyrosensor_object_object_info);
...
G_DEFINE_TYPE(GyroSensorObject, gyrosensor_object, G_TYPE_OBJECT)
/**
...
void gyrosensor_object_emitSignal(GyroSensorObject* obj,
	...
	GyroSensorObjectClass* klass = GYROSENSOR_OBJECT_GET_CLASS(obj);
	...
	/* Check that the given num is valid (abort if not).
	   ...
	   development work). */
	g_assert((num < E_GYROSENSOR_OBJECT_SIGNAL_COUNT) && (num >= 0));
...
{
	printf("gyrosensor1: get_status called.\n");
	g_assert(obj != NULL);

		/* Copy the current first gyrosensor to caller specified memory. */
	if (obj->m_cur_status == DEVICE_STATUS_ERROR)
	{
		gyrosensor_object_emitSignal(obj, E_GYROSENSOR_OBJECT_SIGNAL_ERROR, "error");
	}
	*cur_status = obj->m_cur_status;
