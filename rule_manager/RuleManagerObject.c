 ...
 */
#include "ruleManager-server-stub.h"
 ...
 */
static void ruleManager_object_init(RuleManagerObject* obj) {
	printf("Called");
	g_assert(obj != NULL);
}
 ...
 */
static void ruleManager_object_class_init(RuleManagerObjectClass* klass) {
	...
	/* Since all signals have the same prototype (each will get one
	   ...
	   enumerations before. */
	const gchar* signalNames[E_RULEMANAGER_OBJECT_SIGNAL_COUNT] = {
		SIGNAL_NEW_RULE,
		SIGNAL_ERROR };
	/* Loop variable */
	int i;
	...
	for (i = 0; i < E_RULEMANAGER_OBJECT_SIGNAL_COUNT; i++) {
		guint signalId;
	 ...
	 * the D-Bus. */
	dbus_g_object_type_install_info(RULEMANAGER_TYPE_OBJECT,
			&dbus_glib_ruleManager_object_object_info);
...
G_DEFINE_TYPE(RuleManagerObject, ruleManager_object, G_TYPE_OBJECT)
/**
...
void ruleManager_object_emitSignal(RuleManagerObject* obj,
	...
	RuleManagerObjectClass* klass = RULEMANAGER_OBJECT_GET_CLASS(obj);
	...
	/* Check that the given num is valid (abort if not).
	   ...
	   development work). */
	g_assert((num < E_RULEMANAGER_OBJECT_SIGNAL_COUNT) && (num >= 0));
...
gboolean ruleManager_object_insert_rule(RuleManagerObject* obj, 
								   		gchar *rule_params, 
										gchar **response,
								   		GError** error)
{
	...
	int ret = RULE_MANAGER->InsertRule(rule_params);
...
gboolean ruleManager_object_insert_event(RuleManagerObject* obj, 
								   		 gchar *event_params, 
										 gchar **response,
								   		 GError** error)
{
	...
	int ret = RULE_MANAGER->InsertRule(event_params);
								   			...
								   			GError** error)
{
	...
	int ret = RULE_MANAGER->DeleteRule(ruleId, false); 
	if (ret == -1)
	{
		/* Error */
		//*response = strdup("error");
...
gboolean ruleManager_object_delete_event(RuleManagerObject* obj, 
								   			gchar *eventId, 
											gchar **response,
								   			GError** error)
{
								   				...
								   				GError** error)
{
	...
	RULE_MANAGER->FetchAllRules(response);
